...
import { ChevronDownIcon, CloseIcon, CopyIcon, PlusIcon, TrashIcon, ExportIcon, ImportIcon } from './icons'
...
export default function SettingsModal() {
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'api' | 'data' | 'about'>('api')
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'api' && (
              ...
              <div className="block">
                ...
                <div data-selectable-text className="mt-2 space-y-1.5 text-xs leading-relaxed text-gray-500 dark:text-gray-500">
                  <p>
                    访问{' '}
                    <a
                      href="https://geekai.live/keys"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="font-medium text-blue-600 underline-offset-2 hover:underline dark:text-blue-400"
                    >
                      https://geekai.live/keys
                    </a>
                    ，登录后创建或选择支持 Image2 的分组，将此密钥填写进来。
                  </p>
                  <p>
                    API URL 已固定为 <code className="bg-gray-100 dark:bg-white/[0.06] px-1 py-0.5 rounded">https://geekai.live/v1</code>。接口模式请在生成输入框中选择。
                  </p>
                </div>
              </div>
            </div>
            ...
            {activeTab === 'about' && (
              <div className="flex h-full min-h-[300px] flex-col items-center justify-center pb-8 px-6">
                <div className="flex flex-col items-center">
                  <div className="mb-5 flex h-[88px] w-[88px] items-center justify-center rounded-full border border-gray-200/80 bg-gray-50/50 text-2xl font-bold text-gray-800 dark:border-white/[0.08] dark:bg-white/[0.02] dark:text-gray-100">
                    i2
                  </div>
                  <h4 className="text-[17px] font-bold text-gray-800 dark:text-gray-100">image2即刻生图</h4>
                </div>
